...
const HISTORICAL_PROVIDERS = new Set(["FRANKFURTER"]);
...
export default class CurrencyConverter extends LightningElement {
  ...
  async loadCurrencies() {
    try {
      ...
      this.showToast(
        "Error",
        "Failed to load currencies: " + this.reduceErrors(error),
        "error"
      );
    }
  }
  ...
  get isHistoricalSupported() {
    return (
      HISTORICAL_PROVIDERS.has(this.selectedProvider) ||
      this.resolvedRateDate === null
    );
  }

  get showHistoricalWarning() {
    return (
      this.resolvedRateDate !== null &&
      !HISTORICAL_PROVIDERS.has(this.selectedProvider)
    );
  ...
  get rateDateLabel() {
    if (!this.resolvedRateDate) return "";
    return " on " + this.resolvedRateDate;
  }
  ...
  handleDatePresetChange(event) {
    this.selectedDatePreset = event.detail.value;
  ...
  async fetchPreviewRate() {
    ...
    try {
      this.previewRate = await getExchangeRate({
        ...
        provider: this.selectedProvider,
        rateDate: this.resolvedRateDate
      });
    } catch (error) {
      this.previewRate = null;
  ...
  async handleConvert() {
    ...
    if (this.showHistoricalWarning) {
      this.showToast(
        "Error",
        "Historical rates are not available for this provider. Please select Frankfurter (ECB).",
        "error"
      );
      return;
    }
  ...
  async loadConversionHistory() {
    ...
    try {
      this.conversionHistory = await getConversionHistory({
        recordLimit: this.historyLimit
      });
    } catch (error) {
      this.showToast(
        "Error",
        "Failed to load history: " + this.reduceErrors(error),
        "error"
      );
    } finally {
  ...
  async handleRowAction(event) {
    ...
    if (action.name === "delete") {
      try {
        ...
        this.showToast(
          "Error",
          "Failed to delete: " + this.reduceErrors(error),
          "error"
        );
      }
    }
